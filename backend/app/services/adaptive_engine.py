...
class AdaptivePsychometricEngine:
    ...
    def start_session(self):
        ...
        # Default audit factors (fully compliant)
        self.audit_factors = {
            trait: {
                "bias_mitigation": 1.0,
    ...
    def answer_question(self, trait: str, question_id: int, score: float):
        ...
        self.user_responses.setdefault(trait, []).append({"question_id": question_id, "score": score})
    ...
    def next_question(self, trait: str) -> Dict:
        """
        Adaptive branching:
        - If confidence < 0.7 → serve next high-weight unanswered question.
        - If confidence >= 0.7 → stop branching.
        """
        if trait not in self.question_bank:
            return {}
        confidence = self.trait_confidence.get(trait, 1.0)
        if confidence < 0.7:
            answered_ids = [r["question_id"] for r in self.user_responses.get(trait, [])]
            remaining = [q for q in self.question_bank[trait] if q["id"] not in answered_ids]
            if not remaining:
                return {}
    ...
    def calculate_ethical_multiplier(self, trait: str, confidence: float) -> float:
        """Multiplicative ethical integration: Π (factor ^ weight)."""
        factors = self.audit_factors.get(trait, {})
        multiplier = 1.0
        for factor_name, weight in self.RESPONSIBLE_AI_FACTORS.items():
            value = confidence if factor_name == "general_quality" else factors.get(factor_name, 1.0)
    ...
    def finalize_trait_score(self, trait: str) -> Dict:
        ...
        if not responses:
            return {"score": 0, "confidence": 0, "audit_profile": self.audit_factors.get(trait, {})}

        raw_average = self.trait_scores[trait] / len(responses)
        ...
        final_score = round(normalized_score * ethical_multiplier, 3)
        self.trait_ai_scores[trait] = final_score

        return {"score": final_score, "confidence": round(confidence, 3), "audit_profile": self.audit_factors.get(trait, {})}

    def generate_report(self) -> Dict[str, Dict]:
        """Generate full explainable Responsible AI report."""
        return {trait: self.finalize_trait_score(trait) for trait in self.question_bank}